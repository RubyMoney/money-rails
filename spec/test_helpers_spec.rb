    ...
    describe "monetize matcher" do
      shared_examples "with monetize matcher" do
        ...
        it "matches model attribute with currency specified by :with_currency chain" do
          expect(subject).to monetize(:bonus).with_currency(:gbp)
        end
        ...
        it "does not match non existed attribute" do
          expect(subject).not_to monetize(:price_fake)
        end

        it "does not match wrong currency iso" do
          expect(subject).not_to monetize(:bonus).with_currency(:usd)
        end

        it "does not match wrong money attribute name" do
          expect(subject).not_to monetize(:bonus).as(:bonussss)
        end
      end
