  ...
  describe ".serve" do
    ...
    context "without authorization" do
      it "prevents pushing" do
        expect { Gemstash::GemPusher.new(nil, gem_contents).serve }.to raise_error(Gemstash::NotAuthorizedError)
        expect { Gemstash::GemPusher.new("", gem_contents).serve }.to raise_error(Gemstash::NotAuthorizedError)
    ...
    context "with invalid authorization" do
      it "prevents pushing" do
        expect { Gemstash::GemPusher.new(invalid_auth_key, gem_contents).serve }.
    ...
    context "with invalid permission" do
      it "prevents pushing" do
        expect { Gemstash::GemPusher.new(auth_key_without_permission, gem_contents).serve }.
          to raise_error(Gemstash::NotAuthorizedError)
        expect(deps.fetch(%w(example))).to eq([])
      end
    end
    ...
    context "with an unknown gem name" do
      it "saves the dependency info and stores the gem" do
        ...
        expect(deps.fetch(%w(example))).to eq([])
        Gemstash::GemPusher.new(auth_key, gem_contents).serve
    ...
    context "with a yanked version" do
      ...
      it "rejects the push" do
        expect { Gemstash::GemPusher.new(auth_key, gem_contents).serve }.
